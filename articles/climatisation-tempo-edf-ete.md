...
Puisque le rouge disparaît, le risque de facture salée s'efface. Mais une mécanique reste active toute l'année, y compris en plein mois d'août : la distinction entre **heures pleines (HP)** et **heures creuses (HC)**.
...
## Combien consomme un climatiseur ? Quelques ordres de grandeur

Impossible de donner un chiffre universel : la consommation d'un climatiseur dépend de la surface à rafraîchir, de l'isolation du logement, du modèle (mono-split, multi-split, mobile) et de sa classe énergétique. On raisonne donc en **ordres de grandeur**, à ajuster selon votre installation :

| Type d'usage clim | Consommation indicative |
|-------------------|-------------------------|
| Petit climatiseur mobile (une pièce) | environ 0,7 à 1,2 kWh par heure de fonctionnement |
| Split réversible bien dimensionné | sensiblement moins à puissance frigorifique égale |
| Rafraîchissement d'une nuit entière | dépend fortement de la consigne et de l'inertie |

Ces valeurs sont **purement indicatives** : un appareil récent et bien dimensionné consomme nettement moins qu'un vieux climatiseur mobile pour le même confort. Le bon réflexe est de relever votre propre consommation. Si vous êtes équipé d'un compteur communicant, notre article [suivre sa consommation avec Linky en Tempo](/blog/linky-tempo-suivre-consommation) explique comment isoler le poste clim dans vos relevés.

## Quand faire tourner la clim cet été ?

Voici la stratégie estivale, simple et déculpabilisante :
...
- **Consigne raisonnable** : visez un écart de 5 à 7 °C maximum avec l'extérieur. Descendre trop bas la température fait exploser la consommation pour un confort discutable.
- **Mode nuit / éco** : la plupart des appareils proposent un mode silencieux et économe qui suffit largement pour dormir.
...
Privilégiez les heures creuses, de 22h à 6h, où le tarif est le plus bas. Rafraîchir le logement la nuit présente un double avantage : l'air extérieur est plus frais, donc la clim consomme moins, et l'électricité coûte moins. Grâce à l'inertie, la pièce reste agréable une partie de la matinée, ce qui permet souvent de couper l'appareil pendant les premières heures pleines de la journée.

### Combien consomme une climatisation sous Tempo ?

Cela dépend entièrement de la surface, de l'isolation et du modèle : on parle en estimations, de l'ordre de 0,7 à 1,2 kWh par heure pour un petit appareil mobile, et sensiblement moins pour un split réversible récent bien dimensionné. Le mieux est de relever votre propre consommation via votre compteur communicant. Comme l'été est presque exclusivement bleu, cette consommation est facturée au tarif le plus bas, modulé seulement par les heures pleines et creuses.
