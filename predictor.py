...
def predict_day(target_date: date, weather: dict | None = None,
    ...
    if couleur != "ROUGE" and remaining["ROUGE"] > 0:
        ...
        if _red_slack <= 1 and (target_date.month >= 11 or target_date.month <= 3):
            ...
            # slack = 1 : un seul jour de marge → force sauf si trop chaud (> 8°C),
            ...
            # Mars 2026 : EDF a placé ROUGE même par temps relativement doux,
            # seuil abaissé de 10°C à 8°C pour capturer ces cas.
            if _red_slack <= 0 or temp_moy <= 8:
                couleur = "ROUGE"
            ...
            # forcer ROUGE sur des jours > 8°C où le score_risque est bas.
            if temp_moy > 8:
                _density_reduction = 0  # aucune réduction au-dessus de 8°C
            elif temp_moy > 5:
...
def _result(target_date: date, couleur: str, score: float,
    ...
    result = {
        ...
        "temp_moy_prevue": weather.get("temp_moy") if weather else None,
        "pression_prevue": weather.get("pressure") if weather else None,
        "humidity_prevue": weather.get("humidity") if weather else None,
        "wind_speed_prevue": weather.get("wind_speed") if weather else None,
        ...
        conn.execute(
            """INSERT INTO predictions
               (date, couleur_predite, probabilite_bleu, probabilite_blanc,
                ...
                temp_moy_prevue, pression_prevue, humidity_prevue, wind_speed_prevue,
               ...
               ON CONFLICT(date, horizon) DO UPDATE SET
                   ...
                   probabilite_bleu = excluded.probabilite_bleu,
                   probabilite_blanc = excluded.probabilite_blanc,
                   probabilite_rouge = excluded.probabilite_rouge,
                   score_risque = excluded.score_risque,
                   temp_min_prevue = excluded.temp_min_prevue,
                   temp_max_prevue = excluded.temp_max_prevue,
                   temp_moy_prevue = excluded.temp_moy_prevue,
                   pression_prevue = excluded.pression_prevue,
                   humidity_prevue = excluded.humidity_prevue,
                   wind_speed_prevue = excluded.wind_speed_prevue,
                   jours_rouges_restants = excluded.jours_rouges_restants,
                   jours_blancs_restants = excluded.jours_blancs_restants,
                   raison = excluded.raison,
                   timestamp_prediction = excluded.timestamp_prediction,
                   score_temperature = excluded.score_temperature,
                   score_budget = excluded.score_budget,
                   score_weekday = excluded.score_weekday,
                   score_gradient = excluded.score_gradient,
                   score_clustering = excluded.score_clustering,
                   score_rte = excluded.score_rte,
                   score_temperature_raw = excluded.score_temperature_raw,
                   score_budget_raw = excluded.score_budget_raw,
                   score_weekday_raw = excluded.score_weekday_raw,
                   score_gradient_raw = excluded.score_gradient_raw,
                   score_clustering_raw = excluded.score_clustering_raw,
                   score_rte_raw = excluded.score_rte_raw,
                   cycle_id = excluded.cycle_id,
                   couleur_precedente = excluded.couleur_precedente,
                   simulated = excluded.simulated,
                   confirmed = excluded.confirmed,
                   couleur_originale = excluded.couleur_originale""",
            (pred["date"], pred["couleur_predite"],
             ...
             pred.get("temp_moy_prevue"), pred.get("pression_prevue"),
             pred.get("humidity_prevue"), pred.get("wind_speed_prevue"),
